...
standalone SVT-AV1 in Quick Convert and SVT-AV1 chunks in av1an. Restoring that
workflow separation does not complete the remaining encoder integrations.

## Implementation order

1. Add an encoder identity independent of the workflow. Keep existing serialized
...
2. Make the output plan describe the codec, decoded pixel format, HDR policy,
...
3. Extract direct drivers for discovery, capability checks, input/output arguments,
   ...
   handles. Add `aomenc`, `vpxenc`, `x264`, and `x265` individually.
4. Prove intermediate timing before exposing each driver. IVF provides a starting
   point for AOM/VPX. H.264/HEVC elementary streams require their own timing path;
